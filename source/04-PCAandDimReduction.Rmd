...
In addition to the "high dimensionality" expected for single-cell data, we expect the data to be both "sparse" and "noisy", as many genes will either not be expressed or not measured in many of the cells -  due to biological variability and practical limitations of both capture and sequencing depth [source](https://ouyanglab.com/singlecell/basic.html#a-gentle-introduction-to-dr). 
...
Note the specification of `normalization.method = 'SCT'` is different than the vignette.


# Summary

Now that we have selected a number of principal components that likely correspond to biological variation of interest, we will use that number as a parameter in the next section, along with our integrated results, to generate clusters.
