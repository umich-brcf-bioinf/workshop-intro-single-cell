...
This enables similar perspectives to bulk plus a few more:
...
broadly applicable, it's helpful to ground the conversation in aspecific 
...
F) cDNAs are undergo library prep and are sequenced
...
specifics of the steps depend on the platform and the specific libary prep 
...
C) The beads are covered with a lawn of milions of oligos. Each oligo is 
...
- For a single droplet, the cDNA barcodes will all match. Barcodes will be 
...
sequencing
C) The sequencer call bases for each sequence. For a large sequencing run, this
...
E) Specialized software bins each distinct barcode into a a putative cell and
...
together in the aqueous flow to create soup of ambient RNA. This appears as a 
...
To reliably interpret the sequence output we need to solve three problems:

<table class='fig'><tr><th class='fig'>Interpreting 10x sequence output </th></tr>
<tr><td class='fig'>![](images/00A-OrientingOnScRNASeq/primary-analysis-qc.png)</td></tr>
<tr><td class='fig'>
A) How do we associate each sequence with an individual cell?
B) How do we align and quantify the sequences to create an expression matrix?
C) How do we distinguish good cells from problem edge cases?
</td></tr>
</table>

To expedite the primary analysis and QC tasks above, 10x Genomics provides the
tool Cell Ranger, which we will discuss in the next lesson.
...
bulk RNA-Seq.
- scRNA-Seq experiments leverage existing NGS approaches by adding a molecular 
label to each mRNA, enabling us to connect all the mRNA's from a cell to a 
distinct cell of origin computationally following sequencing.
