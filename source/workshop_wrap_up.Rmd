...
- RStudio workshop compute environment will be available until **`r rstudio_server_enddate`**.
...
- You can download files from the workshop environment from your terminal/command line window as below. 
  ...
  scp -r YOUR_USERNAME@`r ssh_download_dns`:"ISC_R*" .
  ```
...
#### Downloading Seurat inputs
  ...
  - The Seurat inputs (cellranger triples and dbcells files) used in this 
    ...
    # Curl download a ~2 Gb tarball
    # The curl command below is split over two lines, but you copy/paste them as a single block
    curl -o workshop_isc_inputs.tgz \
        `r aws_s3_bucket` 
