...
## Single-cell publication methods
<details>
<summary>Methods</summary>
... details about model systems, sample metadata, treatment, tissue acquisition, dissociation, library preparation, sequencing platform/versions/parameters, etc. ...

**Data analysis:** All analysis and graphics were generated in R (v4.4.1) [1]. Analysis was performed primarily using the Seurat package (v5.0.1) [2]. Filtered CellRanger (v7.2.0) outputs for each sample were loaded. Cells with extreme values (which indicate low complexity, doublets, or apoptotic cells) were excluded by filtering to include only cells where Genes/cell >300 and % mitochondrial < 15% resulting in approximately 600-5,600 cells per sample after filtering. Counts were then normalized using the SCTransform method with default parameters [3].
