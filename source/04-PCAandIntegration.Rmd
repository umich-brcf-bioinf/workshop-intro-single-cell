...
In addition to selecting a reduced number of PCs to represetn our data, we can also visualize , similarly to how we often look at samples for bulk RNA-seq, with the `DimPlot()` function:
...
Prior to clustering our data using our selected number of PCs, we need to integrate our data across all samples. Having previously used `SCTransform()` to normalize our data select variable and now generated a PCA reduction, we can now use the `IntegrateLayers()` function to do that.

New to Seurat v5 is the improvements to make selecting different integration methods much easier. The results of each alternative integration method are stored within the same `Seurat` object, which makes comparing downstream effects much easier.  So, if we wanted to run the slightly faster `RPCAIntegration` method, we would run (**but we won't here**):
