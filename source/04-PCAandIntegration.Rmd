...
One of our ultimate goals is to cluster the cells according to their expression profiles, and determine what cell types correspond to these clusters for the purposes of differential expression between clusters and/or between conditions within clusters.  However, across the 12 samples in our dataset over 20,000 cells are assayed with approximately 21K genes measured per cell.
...
<!--- <Consider moving to later or to a dropdown and adding additional figure to show loadings> However, as we can see in the example below, genes near the end of each line are those with the greatest influence on the direction and length of the PC.-->


We'll skip the process of calculating the score for each gene and cell per PC, but after running PCA on our data result in a score for each cell for each PC based on the expression of the contributing genes and each gene will have a weight or loading for a given PC. 
...
```{r, pca_heatmap_plot}
# Heatmaps 
DimHeatmap(geo_so, dims=1:30, cells=500, balanced=TRUE, reduction = 'unintegrated.sct.pca')
```
...
<!--- <Move this section to a dropdown>
...
Prior to using our selected number of PCs to cluster our data, we need to integrate our data across all samples. Having previously used `SCTransform()` to normalize our data select variable and now generated a PCA reduction, we can now use the `IntegrateLayers()` function to do that.
