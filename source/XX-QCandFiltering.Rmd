...
As discussed at the start of the workshop, single-cell experiments using 10x Chromium instrument aim to have droplets with one cell plus one bead. However this is an inherently imperfect process and there are other considerations like how healthy or intact the cell was at the time of measurement.
...
<!--As Matt may have discussed--> CellRanger tries to draw a line between cells and empty beads (or equivalent), which we can see in the barcode rank pot from the web summary while we want to draw lines between single, well measured cells and doublets or poorly measured cells/non-cells.
