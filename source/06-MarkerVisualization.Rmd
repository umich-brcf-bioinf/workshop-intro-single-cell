...
After generating clusters, we need to perform differential expression analysis to identify the genes that distinguish those clusters. This should allow us to get visibility on some key questions for our clusters, as [highlighted by the HBC materials](https://hbctraining.github.io/Intro-to-scRNAseq/lessons/09_merged_SC_marker_identification.html), namely:
...
<details>
    ...
    The Ouyang Lab has a [section of their tutorial](https://ouyanglab.com/singlecell/clust.html#sec:diffexpr) that discusses the methods available for differential expression including some highlighted in the figure below, as well as a more extensive section on [threshold considerations](https://ouyanglab.com/singlecell/clust.html#sec:diffexpr), while the [HBC section on marker genes identification](https://hbctraining.github.io/Intro-to-scRNAseq/lessons/09_merged_SC_marker_identification.html) highlights the different types of marker identification options available via Seurat.
</details>
<br>

## Marker identification

First, we'll change identities of the cells to "integrated.sct.rpca.clusters" explicitly with `SetIdent()`. The "seurat_clusters" column is the default column for cell identities and changes each time a new clustering is performed. Recall that we've performed integration and clustering based on the **scale.data** layer. For the DE comparisons used in finding markers, we need to use the **counts** layer. 

Before we run DE on the counts layer - we need to run the `PrepSCTFindMarkers()` command. We'll start that now and then explain what it's doing below.

```{r, prepsctfindmarkers, cache = TRUE, cache.lazy = FALSE, warning = FALSE, message = FALSE}
# =========================================================================
...
```
Recall that in the [Normalization](03-Normalization.html) lesson, we executed 
`SCTransform`. For each sample, SCTransform looped over all genes and built a 
model that normalized the cell-specific depths to a uniform depth for that sample. 
It then used that model "in reverse" to calculate the **SCT counts** layer. This 
would be sufficient if we had only one sample or only wanted to compare counts 
within a sample.


<a class='dbox' href='images/sct_figures/prep-sct-find-markers-schematic.png'>
<img alt='PrepSCTFindMarkers' title='PrepSCTFindMarkers' src='images/sct_figures/prep-sct-find-markers-schematic.png' class='hover-shadow'/>
</a>
...
A. Plot A shows the depth distributions of **SCT counts** across samples. Since 
we have many samples and the counts were recalculated on a 
sample-by-sample basis, any comparison *across samples* would be confounded by 
the inconsistent sample depths.

B. The function [PrepSCTFindMarkers](https://satijalab.org/seurat/reference/prepsctfindmarkers) 
uses the minimum median count (red line in plot A) as the new standard depth for 
all samples and re-corrects the **SCT counts**. 

C. Now the **SCT counts** are normalized to a uniform depth and counts can be 
compared across samples. 

<br/>

Next we'll run  [FindAllMarkers()](https://satijalab.org/seurat/reference/findallmarkers) to generate comparisons between each cluster and all other cells, regardless of the experimental group. Note - the statistical test to perform can be specified in `FindAllMarkers()`, but the default is a Wilcoxon test. <!--- `PrepSCTFindMarkers` uses BPCell data on disk so requires that the integrated data loaded from the .rds file were generated with the updated directory structure (e.g. prepared_data/bpcells) --->
...
![Seurat after PrepSCTFindMarkers.](images/seurat-schematic/seurat-prep-sct-find-markers.png)
...
We can also visualize how individual - like specific genes - or collective features - like metrics in our metadata are expressed across our dataset. For example, we can use `FeaturePLot()` to visualize the percentage of mitochondrial genes expressed per cell to determine if there might be a pattern to where more cells are showing stress.
...
This approach can also be useful to apply to specific gene markers of interest, particularly if there are rarer cell types or subtypes that you want to ensure were segregated into their own cluster with the number of PCs used. 

### Plot object clean-up

Before moving on to the next section, we'll clean up our environment to manage our memory usage: 
