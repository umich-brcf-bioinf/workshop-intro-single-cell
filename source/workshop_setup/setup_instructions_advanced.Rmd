---
title: "Intro to Single-Cell Analysis: Advanced setup instructions"
...
- Note that these instructions are focused on R/RStudio and dependent libraries 
  (e.g. Seurat). Separate instructions on downloading and installing Cell Ranger
  can be found here: 
...
**Install**. The installer will show a progress bar; the process takes
about 2 minutes; click **Finish** when prompted to close the installer.
...
defaults (by repeatedly clicking **Next**) and when prompted, click
**Install**. The installer will show a progress bar; the process takes
less than one minute; click **Finish** when prompted to close the
installer.
...
1.9 Proceed to [Installing required libraries] below.
...
defaults (by repeatedly clicking **Continue**) and when prompted, click
**Install**. The installer will prompt you to confirm your
username/password. The installer will show a progress bar; the process
takes about 1 minutes; click **Finish** when prompted to close the
installer.
...
**Keep Both**.) The installer will prompt you to confirm your
username/password.
...
**Open**. This will launch a new RStudio window. The RStudio window is
divided into several panes. The lower left pane shows the **Console**
...
1.8 Proceed to [Installing required libraries] below.
...
3.1  The workshop exercises requires the installation of special R
  ...
  - Installation program may occassionally present an transient error during the 
    process. Often times, the installer can actually recover from this problem
    so you really only need to pay attention to the message printed
    when the installer completes.
  - Note: These top-level installations automatically trigger the
...
```{r install_basic_packages, eval=FALSE}
# -------------------------------------------------------------------------
...
basic_packages = c("sessioninfo", "here", "tidyverse", "ggrepel", "RColorBrewer")
missing_packages <- setdiff(basic_packages, rownames(installed.packages()))
BiocManager::install(missing_packages, update=FALSE, ask=FALSE)

```

```{r install_advanced_packages, eval=FALSE}
# -------------------------------------------------------------------------
# Install the advanced packages

BiocManager::install("remotes")
BiocManager::install("devtools")
BiocManager::install("satijalab/seurat@seurat5")
BiocManager::install("satijalab/seurat-data@seurat5")
BiocManager::install("satijalab/seurat-wrappers@seurat5")


# -------------------------------------------------------------------------
# BPCells allows data to live on disk instead of in RAM
# https://github.com/bnprks/BPCells
BiocManager::install("bnprks/BPCells/r")


# -------------------------------------------------------------------------
# scCATCH enables automatic annotation on single-cell cluster cell types 
...
```{r install_optional_packages, eval=FALSE}
# =========================================================================
# OPTIONAL libraries
