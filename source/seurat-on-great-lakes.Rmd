...
On Great Lakes, you launch a RStudio session on a Slurm **worker node**. The 
worker node can run RStudio, a browser, or other programs/tools. You 
access the worker node through your browser using a tool call **NoVNC**. 
Accessing the worker node through the browser is a convenient way to use a GUI
...
> to launch SLURM worker nodes directly from the Mac Terminal or MS Command Prompt.
...
- You can us **launch templates** to save common launch configs.  
