...
We already note that the Day0 samples appear to have systematically lower features detected per cell. Let's continue with the `nCount_RNA` plot.
...
We obesrve a similar pattern, where the Day0 samples appear to have lower counts per cell than their Day7 and Day21 counterparts. There is even an outlier cell in HODay0replicate1, with over 100K unique UMIs detected. This cell is very likely a doublet. Finally, lets plot the `percent.mt`.
