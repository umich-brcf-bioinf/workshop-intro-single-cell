...
<tr class='fig'><td class='fig'>A. Cells are grouped together based on similar gene expression profiles. <br/>
B. Cells originally in n-dimensional space (n = num of Principal Components) are projected down to two-dimensional space for visualization.
</td></tr>
</table>
<br/>
...
```{r, find_clusters, cache = TRUE, cache.lazy = FALSE, warning = FALSE, message = FALSE}
# generate clusters
...
In the plot at left, we see that while there are distinct clusters, those clusters seem to stratified by day. This suggests that without integration, these batch effects could skew the biological variability in our data. While on the right, the days are more mixed.
