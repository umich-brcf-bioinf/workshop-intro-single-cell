...
The **assays** in a Seurat v5 object can store data in **layers**; a layer is the data for a single sample.  Initially there is only a single layer, can separate the sample-wise data into layers like so:
...
We see the 12 layers containing the count data for each of the samples. Our Seurat object now looks like this:

![Seurat splitting the counts layer of the RNA assay by sample.](images/seurat-schematic/seurat-split-layers.png)
