...
(e.g. scripts or plots) between your workstation and Great Lakes.

1. _In your workstation's  browser_, open the [OOD Dashboard](https://greatlakes.arc-ts.umich.edu/){target="_blank"}. 
...
2.  OOD will display the contents of your home directory. You can click on a 
    directory to see its contents.
   ![](images/seuratOnGreatLakes/ood-files-browse.png)
3. To view a plot graphic, you can click on the hamburger and then select **View**.
   This will open the plot in a new browser tab.
![](images/seuratOnGreatLakes/ood-files-view.png)
4. You can download one or more files by selecting their checkboxes and clicking the **Download** button. (Note: if you select a directory and click **Download** OOD will download the contents as a single zipped file.)
...
`scp` is a lot like `cp` but it allows you to copy across the network.

To transfer from your workstation to Great Lakes:

1. From you workstation terminal or command window, `cd` into the directory that 
   contains your data. 
2. Adjust the `scp` command below to match the correct source directory and 
  ...
  ```
3. When prompted, enter your uniqname and password.


And you can transfer files from Great Lakes to your workstation. From you 
workstation terminal or command window, Adjust the `scp` command below to match 
the correct source file and uniqname and hit _Enter/Return_ to execute.
