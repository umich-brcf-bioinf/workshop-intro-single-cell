...
Our final Seurat object:

![Final Seurat](images/seurat-schematic/seurat-done.png)

---

# Key decision points in analysis and methodology

<br/> We've successfully analyzed our single-cell RNA-seq data, running the following steps.

Note that the parameters we selected for the workshop are **in bold**. When 
running a different dataset, you will need to review and adjust these choices as 
appropriate.

| Step | Goal | Parameters/tools used in workshop &nbsp; &nbsp; &nbsp;|
|:----------------|:-----------------------|:-----------------------|
| [Loading data](01-GettingStarted.html#Read_in_data) | Initialize Seurat object | `Read10x()`: you will need to specify the **paths to CellRanger directories** that contain the feature_barcode_matrix "triples" |
| [Secondary QC filtering](02-QCandFiltering.html#Removing_low-quality_cells) | Identify healthy, single-cells for downstream analysis | **nFeature_RNA < 300** (genes per cell), **max `percent.mt` < 15** (% mitochondrial genes per cell) |
| [Normalization](03-Normalization.html#Normalization) | Separate biological effects from technical effects | `SCTransform` with default parameters |
| [Integration](04-PCAandIntegration.html#Integrate_Layers) | Remove batch effects between individual or groups of samples | `IntegrateLayers` function, using `RPCAIntegration` as the method |
| [Clustering](05-ProjectionAndClustering.html#Choosing_the_number_of_PCs_to_represent_our_data) | Group populations of cells with similar expression programs that correspond to cell-types and/or sub-types of interest | `FindNeighbors` and `FindClusters` using default Louvain algorithm with **10 PCs** and **0.4 resolution** |
| [Annotation](https://umich-brcf-bioinf.github.io/workshop-intro-single-cell/release/html/07-CellTypeAnnos.html#Using_scCATCH) | Identify cell-types present in data | `scCATCH` to generate predicted cell-types and expression plots of marker genes **manually pulled from the literature to finalize the annotations** |
| [Differential expression comparisons](08-DifferentialExpression.html#Differential_Expression) | Identify genes that are impacted by the experimental groups/condition within a given cell-type | `FindMarkers` **with `wilcox` as the statistical test** used for the standard comparisons **and `DESeq` used for the psuedobulk comparisons** |
...
# sessioninfo::session_info() for the nicer formatting. 
# Note, you may have to install the sessioninfo package:
...
-   Trajectory or Pseudotime analysis - see [Single Cell Best Practices: Inferring Trajectories](https://www.sc-best-practices.org/trajectories/pseudotemporal.html) or [Broad Institute's materials on trajectory analysis](https://broadinstitute.github.io/2020_scWorkshop/trajectory-analysis.html)
