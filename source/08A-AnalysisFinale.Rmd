...
|:-----------------------|:-----------------------|:-----------------------|
| Secondary QC filtering | Identify healthy, single-cells for downstream analysis | min and max `nFeature_RNA` (genes per cell), min and max `nCount_RNA` (total counts per cell), max `percent.mt` (% mitochondrial genes per cell) |
...
*This info is generally not included in main body of a paper, but serious reproducibility street-cred if it shows up in supplemental.*<br/> Hear me now, believe me later.

<!-- consider converting to executed code instead of static -->
<!-- consider creating a detailed getting help section that includes a session info example -->
```         
...
|:-----------------------------------|:-----------------------------------|
| Secondary QC filtering | If you see **poor clustering (one big blob) or clusters that seem be driven by quality metrics** like total counts or correspond to stress/debris instead of biological relevant cell-types, **re-filtering with more stringent thresholds might help** |
...
| Clustering | <!-- add PCA and resolution parameter choices--> | <!--Lieden as alternative --> |
...
| Differential expression | We also choose a general cutoff to call DE genes but if there are not enough DE genes, it might make sense to relax that cutoff for some clusters. Conversely, if there are too many DE genes to use for downstream steps like functional enrichments it might be necessary to make the cutoffs more stringent for those specific comparisons. | 
| Subclustering | While we weren't able to cover this approach, if you want to know what general cell-types present but are more interested in comparing the proportions or gene expression between subtypes of a more general cell population (e.g. it's helpful to know how many B-cell, T-cells, etc are generally in your PBMC samples but you are most interested if the proportion of different effector T-cells are impacted by the treatment and if so, what genes are differentially expressed within that subpopulation), it might be worth considering running subclustering after initial clustering and annotation. |
