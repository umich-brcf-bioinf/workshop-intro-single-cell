...
# This inserts the marker gene results from Seurat
geo_catch@markergene = geo_markers
```

To increase the speed and accuracy of our predictions, we'll create query only relevant tissues (which requires some prior knowledge of the experiment and using the [scCATCH wiki](https://github.com/ZJUFanLab/scCATCH/wiki) to select tissues from the species) before we run the tool:
...
new_metadata = geo_so@meta.data %>% left_join(catch_celltypes, by = c('integrated.sct.rpca.clusters0.4' = 'cluster'))
