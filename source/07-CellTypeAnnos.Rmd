...
Next, we'll use a revised version of the cell type predictions from a file that we prepared after refining the scCATCH prediction and add those labels to our Seurat object to replace our clusters' numerical identities. 

Note: we will create a new metadata object where we join cell types. However, this will destroy the row names - which will cause a problem in Seurat - so we have to add them back. <!--- modify to have a hidden block that runs after to keep this large block intact -->
...
It can also be helpful to remember what our original UMAP looked like and compare it to our annotated version.

```{r, compare_umap_plots, fig.width = 10, fig.height = 8, fig.show='hold', echo = FALSE, warning = FALSE, message = FALSE}
org_umap_plot = 
