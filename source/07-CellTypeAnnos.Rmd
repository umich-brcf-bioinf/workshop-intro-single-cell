...
```{r, imm_marker_gene_check, fig.show='hold'}
...
# Load the revised annotations from file --------------
celltype_annos = read.csv('results/tables/revised_celltype_annos.csv')
head(celltype_annos)
```
...
# Merge cell types in but as a new table to slide into @meta.data
new_metadata = copy_metadata %>% dplyr::left_join(celltype_annos, by = c('integrated.sct.rpca.clusters' = 'cluster'))
rownames(new_metadata) = rownames(geo_so@meta.data) #  We are implicitly relying on the same row order!
...
catch_umap_condition_plot = DimPlot(geo_so, group.by = 'cell_type', split.by = 'time', label = TRUE, reduction = 'umap.integrated.sct.rpca')
